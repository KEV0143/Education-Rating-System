...
from sqlalchemy import literal, event
...
def check_for_updates() -> None:
    ...
    try:
        remote_version, download_url = _get_latest_release()
...
def index():
    ...
    years = [y[0] for y in db.session.query(Course.year).distinct().order_by(Course.year).all()]
    ...
    if search:
        ...
        for c in query.order_by(Course.title).all():
            results.append({"course": c, "reason": None})
